...
- [ ] Compute rolling correlation and spread between two user-selected tickers
- [ ] Calculate z-score of the spread and trigger long/short signals at ±2σ
- [ ] Backtest the strategy over a configurable lookback window
- [ ] Display spread chart with entry/exit markers and P&L curve

**Files:** `backtest.py`, `screener.py`, new `pairs.py`
...
- [ ] Flag stocks with surprise % above/below configurable thresholds (e.g. ±5%)
- [ ] Track 1-week, 2-week, and 1-month post-earnings price drift
- [ ] Add PEAD signal as an optional overlay in the backtesting engine
- [ ] Surface top PEAD candidates in the screener

**Files:** `factors.py`, `backtest.py`, new `pead.py`
...
- [ ] Fetch dividend history and compute 5yr CAGR and growth streak
- [ ] Add dividend-specific filter criteria to the screener
- [ ] Score dividend quality as a composite (yield + growth + safety)
- [ ] Add a "Dividend Growth" preset to the screener templates

**Files:** `screener.py`, `factors.py`
...
- [ ] Calculate Graham Number from existing EPS and book value per share data
- [ ] Compute margin of safety as (Graham Number − Price) / Graham Number
- [ ] Add as a screener filter and factor score dimension
- [ ] Display Graham Number vs current price in the analysis view
...
- [ ] Compute 6M and 12M total return for all screener universe tickers
- [ ] Rank and bucket into deciles; highlight top and bottom performers
- [ ] Integrate relative strength rank into the composite factor score
- [ ] Add a "Momentum Leaders" screener preset

**Files:** `sectors.py`, `screener.py`
...
- [ ] Implement the 9 Piotroski binary signals (ROA, CFO, ΔROA, accruals, ΔLeverage, ΔLiquidity, dilution, ΔMargin, ΔTurnover)
- [ ] Add F-Score as a 9th factor in the composite model (or standalone quality dimension)
- [ ] Surface high F-Score stocks (≥7) in the screener
- [ ] Show per-signal breakdown in the analysis view
...
- [ ] Extend existing regime detection (20.1) to distinguish Stagflation and Recovery phases
- [ ] Define per-regime factor weight presets (e.g. upweight value in stagflation, growth in bull)
- [ ] Allow users to override regime or set custom weight profiles in `config.yaml`
- [ ] Show active regime and weight adjustments in the dashboard

**Files:** `factors.py`, `config.py`, `config.yaml`
...
- [ ] Encode monthly/quarterly seasonal bias multipliers for each calendar period
- [ ] Apply a small seasonal adjustment (±5 pts) to the composite factor score
- [ ] Backtest seasonal strategies in isolation to validate historical edge
- [ ] Display active seasonal context in the dashboard header
...
- [ ] Track 5 asset-class ETFs alongside the existing sector ETFs
- [ ] Compute equal risk contribution weights based on rolling volatility
- [ ] Generate monthly rebalancing signals and target allocations
- [ ] Add a "Risk Parity" tab to the Portfolio or Sectors page

**Files:** `portfolio_analysis.py`, `sectors.py`
...
- [ ] Add short interest % float and days-to-cover as screener filter fields
- [ ] Flag stocks with insider net selling + weak factor score (< 35) + high short interest
- [ ] Create a "Short Candidates" preset in the screener templates
- [ ] Integrate with existing short squeeze preset to show both sides of the signal

**Files:** `screener.py`, `ownership.py`
