...
if closed_positions:
    ...
    # vs SPY alpha
    pnl_values = [
        p.get("pnl_pct") for p in closed_positions if p.get("pnl_pct") is not None
    ]
    spy_values = [
        p.get("spy_pnl_pct")
        for p in closed_positions
        if p.get("spy_pnl_pct") is not None
    ]
    ...
    col5.metric(
        "Avg P&L %",
        f"{avg_pnl:+.2f}%",
        delta=f"{avg_alpha:+.2f}% vs SPY" if avg_alpha is not None else None,
    )
else:
...
if not open_positions:
    ...
    for pos in open_positions:
        with st.container():
            ...
            score_color = (
                "#1a7f37" if score >= 70 else "#2da44e" if score >= 55 else "#888888"
            )
            c2.markdown(
...
if not closed_positions:
    ...
    rows = []
    for pos in sorted(
        closed_positions, key=lambda p: p.get("exit_at") or "", reverse=True
    ):
        pnl = pos.get("pnl_pct")
        ...
        rows.append(
            {
                ...
                "Exit $": f"${pos.get('exit_price', 0):.2f}"
...
if sufficient.empty:
    ...
    st.progress(
        min(total_closed / 5, 1.0), text=f"{total_closed} / 5 minimum closed positions"
    )
else:
...
if not insufficient.empty:
    ...
    with st.expander(
        f"{len(insufficient)} factor(s) need more closed positions ({total_needed} more closes total)"
    ):
        for _, row in insufficient.iterrows():
            needed = max(0, 5 - int(row["n"]))
            st.caption(
                f"• {row['factor']} — {int(row['n'])} / 5 closes ({needed} more needed)"
            )
...
if n_closed < _NARRATIVE_MIN:
    ...
    st.progress(
        min(n_closed / _NARRATIVE_MIN, 1.0),
        text=f"{n_closed} / {_NARRATIVE_MIN} closed positions",
    )
else:
...
with st.expander("Log a manual signal"):
    with st.form("log_signal_form"):
        ...
        price_in = st.number_input(
            "Entry Price", 0.0, value=0.0, step=0.01, format="%.2f"
        )
        spy_in = st.number_input(
            "SPY Entry Price", 0.0, value=0.0, step=0.01, format="%.2f"
        )
