...
def test_classify_defensive_healthcare():
    from analyzer import classify_stock_type
    assert classify_stock_type("Healthcare", pe_ratio=25.0, div_yield=None) == "Defensive"


def test_classify_defensive_consumer_staples():
    from analyzer import classify_stock_type
    assert classify_stock_type("Consumer Staples", pe_ratio=22.0, div_yield=1.0) == "Defensive"
...
def test_classify_cyclical_financials():
    from analyzer import classify_stock_type
    assert classify_stock_type("Financials", pe_ratio=10.0, div_yield=None) == "Cyclical"


def test_classify_cyclical_industrials():
    from analyzer import classify_stock_type
    assert classify_stock_type("Industrials", pe_ratio=20.0, div_yield=None) == "Cyclical"
...
def test_classify_returns_valid_type():
    ...
    for sector in ["Technology", "Healthcare", "Energy", "Financials", "Utilities", None]:
...
def test_stream_backtest_narrative_live_stream(tmp_path):
    ...
    mock_client.messages.stream.return_value = _make_mock_stream(["Analysis ", "complete."])
...
def test_stream_sector_rotation_narrative_live(tmp_path):
    ...
    from cache import DiskCache
    sector_data = [
        {"ticker": "XLK", "name": "Technology", "score": 75, "phase": "Leading",
         "perf_1m": 5.0, "perf_3m": 12.0, "rsi": 62.0},
        {"ticker": "XLE", "name": "Energy", "score": 45, "phase": "Lagging",
         "perf_1m": -2.0, "perf_3m": -5.0, "rsi": 38.0},
    ]
    mock_client = MagicMock()
    mock_client.messages.stream.return_value = _make_mock_stream(["Sector ", "analysis."])
...
def test_stream_sector_rotation_narrative_from_cache(tmp_path):
    from analyzer import stream_sector_rotation_narrative
    from cache import DiskCache
    sector_data = [{"ticker": "XLK", "name": "Technology", "score": 75, "phase": "Leading",
                    "perf_1m": 5.0, "perf_3m": 12.0, "rsi": 62.0}]
...
def test_stream_fundamental_analysis_uses_stock_type_prompt():
    ...
    with patch("analyzer._get_client", return_value=mock_client), \
         patch("analyzer._disk_cache", MagicMock(get=lambda k: None, set=lambda *a, **kw: None)):
        list(stream_fundamental_analysis("test prompt", stock_type="Growth", use_cache=False))
...
def test_stream_fundamental_analysis_default_prompt_when_no_type():
    ...
    with patch("analyzer._get_client", return_value=mock_client), \
        ...
        list(stream_fundamental_analysis("test prompt", stock_type=None, use_cache=False))
