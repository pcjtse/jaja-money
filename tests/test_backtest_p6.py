...
def test_gross_return_at_least_net_return():
    ...
    from backtest import run_backtest
    result = run_backtest(_make_df(400, "up"), "TEST",
                          entry_threshold=55, exit_threshold=40,
                          lookback_years=1,
...
def test_higher_costs_reduce_net_return():
    ...
    result_low = run_backtest(df, "TEST", entry_threshold=55, exit_threshold=40,
                              lookback_years=1, commission_pct=0.0, slippage_pct=0.0)
    result_high = run_backtest(df, "TEST", entry_threshold=55, exit_threshold=40,
                               lookback_years=1, commission_pct=0.01, slippage_pct=0.005)
...
def test_total_cost_grows_with_trades():
    ...
    result_aggressive = run_backtest(df, "TEST", entry_threshold=51, exit_threshold=49,
                                     lookback_years=1, commission_pct=0.001)
    result_conservative = run_backtest(df, "TEST", entry_threshold=75, exit_threshold=20,
                                       lookback_years=1, commission_pct=0.001)
...
def test_parameter_sweep_custom_ranges():
    from backtest import run_parameter_sweep
    df = _make_df(300)
    result = run_parameter_sweep(df, "TEST",
                                 entry_values=[60, 65],
                                 exit_values=[30, 35],
                                 lookback_years=1)
...
def test_parameter_sweep_invalid_combos_skipped():
    ...
    assert sharpe_val is None or (isinstance(sharpe_val, float) and str(sharpe_val) == "nan")
