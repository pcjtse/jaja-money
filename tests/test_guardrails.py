...
def _default_flags_args(**overrides):
    ...
    defaults = dict(
        ...
        recommendations=[{"strongBuy": 5, "buy": 5, "hold": 2,
                           "sell": 0, "strongSell": 0}],
...
def test_flags_very_negative_sentiment_danger():
    agg = {"net_score": -0.7, "signal": "Bearish",
...
def test_flags_sorted_danger_first():
    ...
    flags = _build_flags(**_default_flags_args(
        earnings=bad_earnings, financials=fin, sentiment_agg=agg,
        hv=65.0, drawdown_pct=45.0, composite_factor_score=15,
    ))
    if len(flags) >= 2:
...
def test_compute_risk_high_risk_conditions():
    result = compute_risk(
        ...
        recommendations=[{"strongBuy": 0, "buy": 0, "hold": 1,
                           "sell": 5, "strongSell": 5}],
        sentiment_agg={"net_score": -0.9, "signal": "Bearish",
