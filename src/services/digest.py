...
Usage:
    from src.services.digest import generate_digest, get_latest_digest, schedule_digest
...
Do not make up price targets or ratings. Focus on what changed and why it matters."""
    ...
    try:
        ...
        narrative = (
            f"Price: ${price:.2f} ({change_pct:+.2f}%). No AI summary available."
        )
...
def run_batch_analysis(api, limit: int = 50) -> dict:
    ...
    for symbol in batch:
        try:
            ...
            log.debug(
                "Batch: processed %s (price=%s, score=%s)", symbol, price, factor_score
            )
        except Exception as exc:
            msg = f"{symbol}: {exc}"
            errors.append(msg)
...
def generate_weekly_report(api, watchlist_tickers: list[str]) -> str | None:
    ...
    ticker_data.sort(key=lambda x: (x["wow_return"] is None, -(x["wow_return"] or 0)))
